...
fn remap(op: &VmByteCode, op_off: u32, reg_off: u16, in_off: u16, out_off: u16) -> VmByteCode {
    ...
    match *op {
        ...
        ADDC { id, dst, src, cst } => ADDC {
            id: i(id),
            dst: r(dst),
            src: r(src),
            cst,
        },
        SUBC { id, dst, src, cst } => SUBC {
            id: i(id),
            dst: r(dst),
            src: r(src),
            cst,
        },
        CSUB { id, dst, src, cst } => CSUB {
            id: i(id),
            dst: r(dst),
            src: r(src),
            cst,
        },
        MULC { id, dst, src, cst } => MULC {
            id: i(id),
            dst: r(dst),
            src: r(src),
            cst,
...
fn replicate(
    ...
    assert!(
        (n_cores as u32) * (n_regs as u32) <= u16::MAX as u32,
        "register id space (u16) exhausted"
    );
    let n_in_slots = n_cores * n_in as usize;
    assert!(
        n_in_slots <= u16::MAX as usize + 1,
        "input id space (u16) exhausted"
    );
    let n_out_slots = n_cores * n_out as usize;
    assert!(
        n_out_slots <= u16::MAX as usize,
        "output id space (u16) exhausted"
    );
    ...
    for c in 0..n_cores as u32 {
        ...
        irs.push(
            stream
                .iter()
                .map(|op| remap(op, op_off, reg_off, in_off, out_off))
                .collect(),
        );
...
fn verify_correctness() {
    ...
    let builder = single_lane(spec);
    let mut pipeline = Pipeline::new()
        .with_builder(builder)
        .with_vm_config(lane_config)
        .with_topology(Topology::single_core());
    let sig = pipeline.get_prototype().to_owned();
    let n_in = sig.get_args_arity();
    let single = pipeline.into_vm_execution_plan();
    let n_out = sig.get_returns_arity();
    assert_eq!(n_in.sas::<usize>(), 3, "expected (from, to, amount) inputs");
    let (plan, _n_in_slots, n_out_slots) = replicate(&single, n_cores, n_in.sas(), n_out.sas());
    ...
    let seed = std::env::var("VM_SEED")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0);
    let n_rounds: usize = std::env::var("VM_ROUNDS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(10);
    ...
    for round in 0..n_rounds {
        ...
        let in_cts: Vec<RadixCiphertext> = cases
            ...
            .collect();
        let mut out_cts: Vec<RadixCiphertext> = (0..n_out_slots)
            .map(|_| ck.encrypt_radix(poison, n_blocks))
            .collect();
...
fn bench_throughput(c: &mut Criterion) {
    ...
    let single = pipeline.into_vm_execution_plan();
    assert_eq!(n_in.sas::<usize>(), 3, "expected (from, to, amount) inputs");
    let (plan, n_in_slots, n_out_slots) = replicate(&single, n_cores, n_in.sas(), n_out.sas());
    println!(
        "Building the replicated plan took {} us",
        t.elapsed().as_micros()
    );
    ...
    let in_cts: Vec<RadixCiphertext> = (0..n_in_slots)
