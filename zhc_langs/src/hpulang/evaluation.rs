...
impl Evaluation for HpuValue {}

impl EvaluatesTo<HpuValue> for HpuTypeSystem {
...
impl Evaluable<HpuValue> for super::HpuInstructionSet {
        ...
        match self {
            ...
            Batch { block } => {
                ...
                for (val, pos) in arguments.iter().zip(arg_positions.iter()) {
                    context.batch_args.insert(*pos, (*val).to_owned());
                }

                // Recursively interpret the nested block.
                block.evaluate(context).expect("Batch evaluation failed");
