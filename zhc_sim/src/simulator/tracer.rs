...
impl Tracer {
    ...
    ///
    /// Recording occurs only if `tracing_level` enables counters. A `group`
    /// places the counter in its own per-group lane, keeping otherwise
    /// identically-named counters from distinct sources separate; `None` uses
    /// the shared default counters lane.
    ...
    pub fn add_event<E: Event>(&mut self, tracing_level: TracingLevel, at: Cycle, event: &E) {
    ...
    /// Records the `state` of a component under `name` at the specified cycle.
    ///
    /// Recording occurs only if `tracing_level` enables states. Consecutive
    /// recordings of an unchanged serialized `state` are coalesced into a single
    /// span. A `group` places the state in its own per-group lane, keeping
    /// otherwise identically-named states from distinct sources separate; `None`
    /// uses the shared default states lane.
